...
public class ReservacionDao implements IReservacionDao, Serializable {
    ...
    public List<Reservacion> obtenerListaReservacion(Reservacion reservacion) {
        ...
        try {
            ...
            if (UtilCadena.cadenaValido(reservacion.getDuserRevisor())) {
                String add="u.duserRevisor= :usuario_revisor";
                if(i>0){
                    add=" AND "+add;                    
                }
                sql=sql+add;
            ...
            if (UtilCadena.cadenaValido(reservacion.getDuserReparador())) {
                String add="u.duserReparador= :usuario_reparador";
                if(i>0){
                    add=" AND "+add;                    
                }
                sql=sql+add;
            ...
            if (UtilCadena.cadenaValido(reservacion.getDuserRevisor())) {
                q.setParameter("usuario_revisor", reservacion.getDuserRevisor());
            }
            if (UtilCadena.cadenaValido(reservacion.getDuserReparador())) {
                q.setParameter("usuario_reparador", reservacion.getDuserReparador());
            }
            l = (List<Reservacion>) q.list();
