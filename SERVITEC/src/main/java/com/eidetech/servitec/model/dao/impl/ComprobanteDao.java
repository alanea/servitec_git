...
import com.eidetech.servitec.model.dao.IComprobanteDao;
...
public class ComprobanteDao implements IComprobanteDao, Serializable {
    ...
    public List<ComprobanteDetalle> obtenerListaComprobanteDetalladoVenta(ParametroVentaDetallada parametro) {
        ...
        if (parametro != null) {
           ...
           if (UtilFecha.fechaValido(parametro.getFrom()) && UtilFecha.fechaValido(parametro.getTo())&& parametro.getFrom().before(parametro.getTo())) {
                i++;
                if (i > 1) {
